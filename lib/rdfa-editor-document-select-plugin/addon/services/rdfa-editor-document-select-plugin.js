...
export default class RdfaEditorDocumentSelectPlugin extends Service {
    constructor() {
        ...
        this.insertDecisionDocumentsKeyword = /:header/g;
    }

    addDocsSelectorHint(hrId, hintsRegistry, editor, rdfaBlock, location, headerRoot){
    ...
    execute(hrId, rdfaBlocks, hintsRegistry, editor) {
        ...
        for (const rdfaBlock of rdfaBlocks) {
            ...
            if (
                ...
                nodeAttributes.properties.includes('https://data.vlaanderen.be/ns/besluitvorming#heeftBijlage')){
                const parent = rdfaBlock.semanticNode.parent.parent;
                if(!parent.rdfaAttributes.properties.includes('https://header')) continue;
